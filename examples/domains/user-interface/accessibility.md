...
**For LLM:** Text a screen reader speaks is still copy, and it is usually the copy nobody edits. It is also the only version of the interface some readers get, which makes an unlabeled control a missing feature for them.
...
Give every control an accessible name that states the action it performs. Where the control has a visible text label, that label's words must appear in the accessible name. Use *Search*, not *magnifier*, for an icon with no visible label. Check the computed accessible name, not only the proposed copy.
...
Write about people first and the disability second, and only where it is relevant — but follow a community's stated preference over the general rule, which is `examples/domains/medical.md`'s call and stands: *autistic person*, *Deaf person*. Say what a person uses or does, not what they lack. Never reach for a disability as a metaphor for a flaw.
...
The text assistive technology reads, on any surface in any medium. Use it alone for a scoped accessibility-copy task, or with the relevant medium guidance for a whole surface. Where the product behavior is not supplied, say so rather than assuming another file was loaded. Reviewed copy is not implemented accessibility.
