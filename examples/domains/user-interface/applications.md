...
**For LLM:** Text inside a product someone is using to get something done. The reader did not open the app to read, and every word is a toll on the way to what they came for.

Platform-neutral house rules. For Apple platforms, load `examples/domains/user-interface/apple-hig.md` as well; it wins on mechanics.
...
- `(s)` and slashes standing in for a real message variant. Select a whole message for each plural category through the localization system rather than concatenating English fragments.
...
Address the person: *you*, *your*. Keep *we* for the company doing something the reader needs to know about, and use it rarely. Present tense suits a current state or an ongoing action; use past tense for an event that already happened. Prefer active voice with the actual actor, and do not make the reader the subject of a failure the system caused.
...
Write about people first and disability second, unless the community has said otherwise — `examples/domains/medical.md` holds that exception and it survives this layer. Never borrow the name of a condition to label something bad.
...
Long-form product writing — release notes, help articles, in-app guides — is `examples/domains/technical.md`. Email and other messages that arrive outside the product are neither this module nor `website.md`; use `examples/domains/` until this axis claims them. Conversational surfaces — an assistant's replies, a chat agent's refusals — are not covered here either: they are read rather than scanned, so the attention economy this module assumes does not hold.
