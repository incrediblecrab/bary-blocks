...
**For LLM:** A chart may support a finding or let a reader explore data. Its words should make the measure and limits intelligible, not supply a conclusion the data cannot support.
...
Write a short text alternative naming the chart type and what it shows, because the encoding carries meaning here — this is the one place `accessibility`'s ban on opening with the medium does not apply. For a complex chart, add a longer description or the underlying data table carrying the scales, values and relationships the reader needs, and keep that structure navigable rather than one continuous sentence. State the finding, the range and the outlier when the data supports them, and invent none of them to fill the pattern.
