...
**For LLM:** Documentation, references, guides, READMEs, changelogs. Pairs with `examples/citations/ieee.md` where formal citation is needed at all.
...
- Use source links or formal citations as the task and venue require. Select `examples/citations/ieee.md` only when IEEE style is requested; technical subject matter does not choose it automatically.
...
Give the failure cases the reader is likely to hit, and a next step where one genuinely follows. Do not append a recap or an offer to continue in order to fill the shape.
...
Against `examples/domains/user-interface/`. A real boundary now that the layer exists. This module documents software; `examples/domains/user-interface/` writes the software's own words. Error message text belongs to `examples/domains/user-interface/applications.md`; the troubleshooting page that explains the error belongs here. Button labels are theirs; the guide that tells the reader which button belongs to this module.
...
With `examples/education-levels/`. Describe the actual reader's familiarity and support needs. Use classroom presets for classroom tasks, not as labels for every novice, expert or person with a degree. Clear beginner instruction keeps the same accuracy and safety standard.
