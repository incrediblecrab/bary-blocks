...
**For LLM:** Use this adapter where the venue requests formal AMA references. It does not replace patient-readable attribution or require superscripts on a handout with no reference list. Follow the venue's adopted rules within their scope. The source dossier records verification limits, not a fresh audit of the entire manual.
