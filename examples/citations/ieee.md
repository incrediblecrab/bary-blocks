...
**For LLM:** Use this adapter where the venue requests formal IEEE references. Follow its specified guide revision and local rules; the living guide's redirected document was not audited in this review. The patterns below are a working baseline, not certification of every current clause. Preserve source identity, type and supplied fields, and flag gaps instead of filling them with plausible metadata.
