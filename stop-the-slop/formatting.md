...
**For LLM:** Formatting should help this reader find and understand the content. Decorative structure can conceal a thin argument, but a list, table or heading is not a defect merely because models also use it. Claims about how training produced a particular habit belong in the evidence dossier, with their limitations.
...
- **Inline-header bullets.** `- **Thing:** description`, repeated down the page where the labels draw no useful distinction.
...
A reference list, such as a taxonomy, an index or a glossary whose items are distinct named things, is not this defect. The fault is a list standing in for prose that had a throughline, not a genuine enumeration of discrete items.
...
Use consistent quotation typography appropriate to the medium. Straight quotes are the plain-text default. Do not normalize quotation marks or apostrophes inside code, identifiers, or message-format patterns, where the character is syntax rather than typography. Keep meaningful notation and accessible links. Avoid decorative symbols that obstruct reading or substitute for structure.
